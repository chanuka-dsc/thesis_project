...
# Load data
fais = pd.read_csv('C:/Users/dhruv/Desktop/Thesis work/Datasets/trajectory-feats/amended/ais-point-feats-extracted.csv')
fais = fais.dropna(subset=['label'])

# labels = sorted(np.unique(y))
# print("✅ Unique labels in y:", labels)

# Filter out labels with less than 2 samples
label_counts = fais['label'].value_counts()
valid_labels = label_counts[label_counts >= 2].index
fais = fais[fais['label'].isin(valid_labels)].copy()

# Encode labels
label_encoder = LabelEncoder()
y = label_encoder.fit_transform(fais['label'])
print("✅ Encoded label mapping:", dict(zip(label_encoder.classes_, range(len(label_encoder.classes_)))))
print("✅ Unique labels in y:", sorted(np.unique(y)))

# Prepare data
imputer = SimpleImputer(strategy='mean')
X = fais.drop(columns=['tid', 'label'], errors='ignore')
X = pd.DataFrame(imputer.fit_transform(X), columns=X.columns)
print("✅ Missing values remaining in X:", X.isna().sum().sum())
...
print(classification_report(y_test, y_pred_dummy, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_dummy])))))
...
print(classification_report(y_test, y_pred_dummy_uniform, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_dummy_uniform])))))
...
print(classification_report(y_test, y_pred_dummy_stratified, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_dummy_stratified])))))
...
print(classification_report(y_test, y_pred_logistic_unscaled, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_logistic_unscaled])))))
...
print(classification_report(y_test, y_pred_logistic, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_logistic])))))
...
print(classification_report(y_test, y_pred_tree, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_tree])))))
...
print(classification_report(y_test, y_pred_forest, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_forest])))))
...
print(classification_report(y_test, y_pred_xgb, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_xgb])))))
...
print(classification_report(y_test, y_pred_knn, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_knn])))))
...
print(classification_report(y_test, y_pred_knn_scaled, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_knn_scaled])))))
...
print(classification_report(y_test, y_pred_svm, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_svm])))))
...
print(classification_report(y_test, y_pred_svm_scaled, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_svm_scaled])))))
...
y_pred_linear_class = np.clip(np.round(y_pred_linear), 0, 5).astype(int)
...
print(classification_report(y_test, y_pred_linear_class, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_linear_class])))))
...
y_pred_linear_scaled_class = np.clip(np.round(y_pred_linear_scaled), 0, 5).astype(int)
...
print(classification_report(y_test, y_pred_linear_scaled_class, zero_division=0, labels=sorted(np.unique(np.concatenate([y_test, y_pred_linear_scaled_class])))))

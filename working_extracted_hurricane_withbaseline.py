...
# Load data
fhurri = pd.read_csv('C:/Users/dhruv/Desktop/Thesis work/Datasets/trajectory-feats/amended/hurricanes-point-feats-extracted.csv')
fhurri = fhurri.dropna(subset=['label'])
...
y = label_encoder.fit_transform(fhurri['label'])
...
X = fhurri.drop(columns=['tid', 'label'], errors='ignore')
...
print(classification_report(y_test, y_pred_dummy_uniform, zero_division=0))
...
print(classification_report(y_test, y_pred_dummy_stratified, zero_division=0))
...
print(classification_report(y_test, y_pred_logistic_unscaled, zero_division=0))
